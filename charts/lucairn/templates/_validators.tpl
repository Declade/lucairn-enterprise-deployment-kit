...
{{- $subcharts := list
  ...
  (dict "name" "pii-ml" "stateDesc" "single-process Python gRPC sidecar (services/pii-ml/server.py) running Piiranha + GLiNER. Models are loaded eagerly at boot into the process's torch/transformers state; multi-replica is structurally untested" "footgun" "operational variance on a load-bearing Phase 7 emitter going into the v1.0 demo lane; circuit-breaker state inside sanitizer is pod-local and would diverge across pii-ml replicas")
-}}
{{- range $sc := $subcharts -}}
