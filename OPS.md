...
- **Partially covered — `lucairn-dashboard`.** It IS checked, but only at
  ...
  unverified until a dedicated `--dashboard-tag` (or equivalent) flag ships
  — see the CLI follow-up ticket referenced below.
...
> **Two distinct behaviors — do not conflate them.** Setting `LUCAIRN_LICENSE_KEY` + `LUCAIRN_LICENSE_PUBLIC_KEY` always drives Enterprise **feature-gating** (the grace-then-degrade lifecycle described below) — that is the stock behavior and works on any image. The hardened **anti-tamper coupling** is a separate, single switch that is active **only** on a **pin-baked release gateway image** (one built with the Ed25519 public key baked into the binary's `PinnedPublicKeyHex` via `-ldflags -X`). On a pin-baked image: the gateway **fail-closes at boot** on a missing/forged entitlement; self-serve `POST /api/v1/register` is **disabled** (`403 registration_disabled`); the `DSA_ENV=development` enforcement bypass is **closed**; and the entitlement's `customer_id` is **coupled** to the customer's gateway keystore so a mismatch returns `403 entitlement_mismatch`. **Stock / committed / GHCR images ship with `PinnedPublicKeyHex=""` and are fully INERT for anti-tamper** — populating the two env vars on a stock image gives you the feature-gating above but **zero** anti-tamper (no fail-closed boot, no key↔entitlement coupling, `/api/v1/register` stays open, `DSA_ENV=development` still bypasses). The pin-baked image is built by **Lucairn** (the operator); a customer building their own gateway needs the gateway Dockerfile's `ARG LUCAIRN_LICENSE_PUBLIC_KEY_HEX` (in dual-sandbox-architecture) and should contact Lucairn for the pin-bake build recipe.
...
The `license issue` / `verify` / `gen-key` subcommands shell out to the DSA `license-sign` tool so the signed bytes are byte-identical to what the gateway verifier reconstructs. Discovery order: `LUCAIRN_LICENSE_SIGN_BIN` (an explicit override path to a prebuilt `license-sign` binary), then `license-sign` on `PATH`, then `go run` from `DSA_REPO` (the dual-sandbox-architecture clone). Set `LUCAIRN_LICENSE_SIGN_BIN` when you ship operators a prebuilt binary and do not want to require a Go toolchain. If none of the three resolves, the command fails cleanly (exit 1) with an actionable pointer.
...
Schema change: none on the DSA stack. No database migration required for self-hosted installs.

All 12 `dsa-*` images are republished + cosign-signed + Rekor-logged at `0.5.4`
(`bin/lucairn verify-images --tag 0.5.4` → 13/13). `dsa-pii-ml` stays `0.5.1`
(independent cadence) and `lucairn-dashboard` stays `0.8.2`.

- **Per-key MCP tool-scope enforcement (INERT by default):** the gateway enforces
  a `tool_allowlist` field from the customer profile — only `mcp__*` data-source
  tools in the allowlist pass through; all other `mcp__*` tools are stripped.
  Empty allowlist (default) is byte-identical to pre-0.5.4 behaviour.
  (DSA PR #303; Approach B scoping — non-`mcp__` tools always pass through.)
...
**Compose:** set `LUCAIRN_IMAGE_TAG=0.5.3` in `customer.env`, then re-run with the SAME `-f` overlay set you normally use — a self-hosted (full on-prem) install MUST include `docker-compose.self-hosted.yml`, or the gateway is recreated with split-deployment settings and the local `sandbox-b` is orphaned:
...
Sanitizer-only behavior change plus a witness-image tooling addition. All 12
`dsa-*` images are republished + cosign-signed + Rekor-logged at `0.5.2`
(`bin/lucairn verify-images --tag 0.5.2` → 13/13). `dsa-pii-ml` stays `0.5.1`
(independent cadence) and `lucairn-dashboard` stays `0.8.2`.

- **A6 strict LOCATION stop-list:** the `0.5.2` sanitizer drops spaCy-NER
  LOCATION false positives on common English words (`West`/`Loop`/`For`) that
  the en `SpacyRecognizer` mis-tagged in messy ITSM/ServiceNow prose, even after
  the 0.5.1 `de_places` en-exclusion. Whole-token-exact, LOCATION-only, spaCy-
  NER-only — multi-word places, longer tokens, PERSON spans, and L1 identity
  surnames stay redacted (recall-safe). Bundled in
  `config/safe-terms-strict-location.txt`, auto-mounted by
  `docker-compose.customer.yml`, and delivered to Helm operators via the
  `sanitizer-config` ConfigMap key `safe-terms-strict-location.txt`.
- **Turnkey `sign-manifest`:** the `dsa-veil-witness:0.5.2` image ships
  `/usr/local/bin/sign-manifest`. The production key-ceremony step (INSTALL § 4b)
  now runs `docker run --entrypoint sign-manifest …` — no Go toolchain, no
  build-from-source, no dev-mode fallback. Closes BLOCKER-3.

**Compose:** set `LUCAIRN_IMAGE_TAG=0.5.2` in `customer.env`, then:
```bash
...
**Helm:** set `global.imageTag: "0.5.2"` (or override `sandbox-a.sanitizer.image.tag`) and apply:
```bash
helm upgrade lucairn charts/lucairn -n lucairn -f your-values.yaml
```
