...
**Development / evaluation:** run `bin/lucairn-init --dev --runtime-mode <mode>` with the mode-compatible required flags: both `--remote-endpoint https://...` and `--remote-credentials <Lucairn-issued-file>` for `split-remote`, no extra runtime flag for `managed-byok`, or `--local-runtime <supported-name>` for `local-runtime`. It generates a full key set and the non-secret `customer.env.runtime-profile.yaml` plus recorded `customer.env.image-manifest.yaml` sidecars automatically. Keep both beside the env: a marker-bearing S1 env fails closed if either is deleted, symlinked, or changed. Non-BYOK modes run doctor during init; managed-BYOK intentionally leaves provider keys empty and skips doctor, so set at least one provider key and run doctor before Compose. No manual ceremony needed.

**Production:** run `bin/lucairn-init --production --runtime-mode <mode> --license license-bundle.json` with the compatible required flags above (or fill `customer.env` by hand following this runbook), then produce and deploy the witness-signed manifest blob (§ 6) before starting the stack.
