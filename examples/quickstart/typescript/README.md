...
## Why raw `fetch` and not a Lucairn SDK?

The Lucairn gateway is Anthropic-Messages-API compatible, so the cleanest
quickstart is one `fetch` call. A published `@lucairn/sdk` for `/v1/messages`
will land later; raw `fetch` is the supported path today and is exactly what
the SDK will wrap.
