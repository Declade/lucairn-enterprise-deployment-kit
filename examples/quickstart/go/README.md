...
Requires Go 1.24+. No third-party dependencies — pure `net/http`.
...
## Why no Lucairn SDK import?

The Lucairn gateway is Anthropic-Messages-API compatible, so the cleanest
quickstart is one stdlib `net/http` call. A published Go SDK module for
`/v1/messages` will land later; raw `net/http` is the supported path today and
is exactly what the SDK will wrap.
...
- This example is a standalone Go module (its own `go.mod`); it deliberately
  does NOT join the repo's `go.work` so users can clone-and-run without
  pulling the whole monorepo's build graph.
