...
**Existing installs keep working unchanged. No migration is required at
runtime.** The gateway, sanitizer, and veil-witness images (v0.5.0+) read
each setting under its `LCR_*` name first, then fall back to the legacy
`VEIL_*` name if `LCR_*` is unset. Customer.env files generated by earlier
kit versions continue to validate via the doctor's dual-name check and
continue to boot the stack cleanly.
...
```bash
# Docker-compose customers — rename the renameable signing/public keys
# in customer.env. (POSTGRES_VEIL_PASSWORD and VEIL_APP_PASSWORD stay.)
sed -i -E 's/^VEIL_(AUDIT|BRIDGE|SANITIZER|WITNESS)_SIGNING_KEY=/LCR_\1_SIGNING_KEY=/' customer.env
sed -i -E 's/^VEIL_(WITNESS_PUBLIC_KEY|WITNESS_KEY_ID|WITNESS_SIGNED_MANIFEST_PATH|WITNESS_MTLS_HOST_DIR|WITNESS_MANIFEST_PUBLIC_KEY)=/LCR_\1=/' customer.env
