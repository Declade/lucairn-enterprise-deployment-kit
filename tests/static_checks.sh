...
# Overnight follow-up 2026-06-15: check_manifest_blob pre-flight (sign-manifest
# BLOCKER-3 containment). Pure unit test — sources bin/lucairn and exercises the
# production-gated witness-signed-manifest pre-flight against temp fixtures. No
# docker/cosign/network needed, so it runs in the static gate.
bash "$ROOT/tests/test_manifest_blob.sh"

# B1 Slice 3: the manifest digest block records EXACTLY the 13 cosign-signed
# artifacts (keys/image-digests-0.5.1.txt) under signed_artifacts, in lockstep.
...
done < "$ROOT/keys/image-digests-0.5.1.txt"
...
echo "B1-S3: image_digests block in lockstep with keys/image-digests-0.5.1.txt (via parser) + dashboard manifest synced"
